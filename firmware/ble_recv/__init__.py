...
Closes a block on ##MORE or 2.8s idle. RAM may fail on huge blocks.
"""
import bluetooth
import gc
import machine
    ...
    for line in text.split("\n"):
        ...
        if s.startswith("##BLOCKS"):
            try:
                ...
                total = 0
        elif s.startswith("##TYPE") or s.startswith("##FMT"):
            p = s.split(None, 1)
            if len(p) > 1:
                ftype = p[1].strip().lower()
        elif s.startswith("##FILE") or s.startswith("##NAME"):
            p = s.split(None, 1)
            if len(p) > 1:
                fname = p[1].strip()
            ...
            more = True
        elif s.startswith("##CONTINUATION") or s == "##END" or s.startswith("##"):
            pass
        else:
            rest.append(line)
...
def draw():
    ...
    display.text("ESC quit", 6, 118, DIM)
    display.show()
